...
func process_attack_state(_delta):
	...
	if vec_to_player.length() <= attack_range and vision_manager.can_see_target(player):
		ai_character_mover.stop_moving()
		if !attacking and vision_manager.is_facing_target(player):
			start_attack()
		elif !attacking and vision_manager.can_see_target(player):
