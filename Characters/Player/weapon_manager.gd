...
func enable_weapon(weapon : Weapon):
	...
	if !weapon_already_unlocked:
		switch_to_weapon_slot(weapon_ind, true)
...
func switch_to_previous_weapon():
	for i in range(weapons.size()):
		...
		if switch_to_weapon_slot(wrapped_ind, false):
			break
...
func switch_to_weapon_slot(slot_ind: int, first_frame : bool)->bool:
	if cur_slot == slot_ind and first_frame == false:
		return false
