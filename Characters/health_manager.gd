...
@export var pick_up_drop : PackedScene
...
func hurt(damage_data: DamageData):
	if get_parent().has_node("AlertSound"):
		get_parent().get_node("AlertSound").stop()
...
func spawn_pick_up()  -> void:
	var drop_inst = pick_up_drop.instantiate()
	get_tree().get_root().add_child(drop_inst)
	drop_inst.global_position = Vector3(global_position.x,global_position.y - 1, global_position.z)
	drop_inst.add_to_group("instanced")
